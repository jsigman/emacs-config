...
#+begin_src emacs-lisp :load yes
(defvar-local unison-sync-queue nil
  "Queue of Unison commands to be processed.")

(defvar unison-sync-process nil
  "Current Unison process, if any.")

(defun unison-sync-enqueue ()
  "Enqueue a Unison sync command, if not already enqueued."
  (unless (member (build-unison-command) unison-sync-queue)
    (push (build-unison-command) unison-sync-queue)
    (unless unison-sync-process
      (unison-sync-dequeue))))

(defun unison-sync-dequeue ()
  "Dequeue the next Unison sync command and execute it."
  (when unison-sync-queue
    (let ((cmd (pop unison-sync-queue)))
      (unison-sync-start-sync cmd))))

(defun unison-sync-start-sync (unison-command)
  "Start Unison sync process using the given command."
  (let ((output-buffer (get-buffer-create "*Unison Sync*")))
    (with-current-buffer output-buffer
      (read-only-mode -1)
      (erase-buffer)
      (insert (format "Running command: %s\n\n" unison-command)))
    (setq unison-sync-process
          (start-process-shell-command
           "unison-sync" output-buffer unison-command))
    (set-process-sentinel unison-sync-process 'unison-sync-sentinel)))

(defun unison-sync-sentinel (process event)
  "Handle completion of a Unison sync process."
    ...
    (if (zerop exit-status)
        (message
         (propertize "Unison sync completed successfully"
                     ...
                     '(:foreground "green")))
      (message
       (propertize (format "Unison sync failed with error: %s" event)
                   'face '(:foreground "red"))))
    (setq unison-sync-process nil)
    (unison-sync-dequeue)))

(define-minor-mode unison-sync-mode
  "Minor mode to sync project with Unison."
  :lighter
  " Unison-Sync"
  (if unison-sync-mode
      (add-hook 'after-save-hook 'unison-sync-enqueue nil t)
    (remove-hook 'after-save-hook 'unison-sync-enqueue t)))
