...
(defvar-local unison-force-root nil
  "Specify which root to force for one-way sync. Should be either 1 or 2.")

(defun build-unison-command ()
  "Build the Unison command based on custom variables."
  (when (and unison-root1 unison-root2)
    (let ((command
           (concat "unison -batch " unison-root1 " " unison-root2)))
      (when unison-force-root
        (setq command
              (concat
               command " -force "
               (if (equal unison-force-root 1)
                   unison-root1
                 unison-root2))))
...
(defun unison-sync-start-sync ()
  "Start Unison sync process."
  (interactive)
  (let ((unison-command (build-unison-command)))
    (when unison-command
           ...
           (lambda (process event)
             ...
             (if (string-match-p "finished" event)
                 (if (zerop (process-exit-status process))
                   ...
                   (message
                    (propertize
                     (format "Unison sync failed with error: %s"
                             event)
                     'face '(:foreground "red"))))
               (message
                (propertize
                 (format "Unison sync failed with error: %s" event)
                 'face '(:foreground "red")))))))))))
...
(define-minor-mode unison-sync-mode
  ...
  " Unison-Sync"
  (if unison-sync-mode
      (add-hook 'after-save-hook #'unison-sync-start-sync nil t)
    (remove-hook 'after-save-hook #'unison-sync-start-sync t)))
