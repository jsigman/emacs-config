...
(use-package markdown-preview-mode
  ...
  (setq markdown-preview-script-onupdate
        "var codes = document.querySelectorAll('pre.mermaid code'); for(var i=0; i<codes.length; i++){ var pre = codes[i].parentElement; pre.innerHTML = codes[i].innerHTML; } mermaid.run();"))
#+END_SRC
