...
(use-package
 ...
 :bind-keymap ("C-c p" . project-prefix-map)
 :config
 (defun my-project-try-remote (dir)
   "Recognize remote projects."
   (when (file-remote-p dir)
     (cons 'remote dir)))

 (add-to-list 'project-find-functions #'my-project-try-remote)

 (cl-defmethod project-root ((project (head remote)))
   (cdr project))

 (defun my-project-remember-remote (remote-dir)
   "Remember a remote project without removing it if not found."
   (project--ensure-read-project-list)
   (let ((dir (abbreviate-file-name remote-dir)))
     (unless (member (list dir) project--list)
 ...
 (advice-add
  ...
  :before
  (lambda (pr &optional no-write)
    (when (and pr (file-remote-p (project-root pr)))
      (my-project-remember-remote (project-root pr)))))

 (advice-add
  'project--remove-from-project-list
  :around
  (lambda (orig-fun project-root report-message)
    (if (file-remote-p project-root)
        (message "Remote project not found: %s" project-root)
      (funcall orig-fun project-root report-message)))))
#+end_src
*** Eglot-iedit
#+begin_src emacs-lisp :load yes
(use-package
