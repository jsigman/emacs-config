...
(use-package
 ...
 ((prog-mode . copilot-mode)
  ...
  (direnv-envrc-mode . copilot-mode)
  (conf-mode . copilot-mode))
 :config
 (setq copilot-max-char -1)
 (setq copilot-indent-offset-warning-disable t)

 ;; Function to check if server is installed and up to date
 (defun ensure-copilot-server-updated ()
   (if (not (file-exists-p copilot--server-executable))
       (progn
         (message "Copilot server not found. Installing...")
         (copilot-install-server))
     (let ((installed-version (copilot-installed-version))
           (required-version copilot-version))
       (when (and installed-version
                  (version< installed-version required-version))
         (message "Updating Copilot server from %s to %s"
                  installed-version
                  required-version)
         (copilot-reinstall-server)))))

 ;; Run the check when the package is loaded
 ;; TODO: Still not working
 ;; (ensure-copilot-server-updated)
...
#+begin_src emacs-lisp :load yes
;;; magit-todos.el --- Display clickable TODOs in Magit status buffer

;;; Commentary:
;; This mode adds a section to the Magit status buffer that displays TODOs found in the repository.
