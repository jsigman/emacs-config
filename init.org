...
(use-package
 ...
 :demand t
 :straight nil
 :init (global-set-key (kbd "C-c p") project-prefix-map)
 :config
 (defun my-project-ignores (dir)
   (let ((root (locate-dominating-file dir ".git")))
     (when root
       (append
        (mapcar
         (lambda (dir) (concat "**/" dir "/**")) ; Use glob patterns
         '("venv"
           "data"
           "typings"
           "node_modules"
 ...
 (cl-defmethod project-ignores ((project (head vc)) dir)
   (let ((root (project-root project)))
     (append
      (project--value-in-dir 'project-vc-ignores root)
      (my-project-ignores dir))))

 ;; Use native indexing for faster performance
 (setq project-vc-extra-root-markers '(".git"))
 (setq project-vc-include-untracked nil)
 (setq project-vc-merge-submodules nil)

 ;; Use git-grep for faster searching
 (setq project-vc-grep-function 'project--vc-git-grep)

 ;; Optimize file indexing
 (setq project-file-list-cache-time 30) ; Cache file list for 30 seconds

 ;; Use fd for even faster file indexing if available
 (when (executable-find "fd")
   (setq project-find-functions '(project-try-vc project-try-fd)))

 (defun project-try-fd (dir)
   (let ((root (locate-dominating-file dir ".git")))
     (when root
       (cons 'fd root))))

 (cl-defmethod project-files ((project (head fd)) &optional dirs)
   (let ((default-directory (cdr project)))
     (project--file-list-fd)))

 (defun project--file-list-fd ()
   (split-string
    (shell-command-to-string
     "fd . -t f -H -E .git -E .gitignore -E '**/venv/**' -E '**/node_modules/**'")
    "\n" t))

 (defun my-project-try-vc (dir)
   (let* ((root (vc-call-backend 'Git 'root dir))
          (check-submodule
           (lambda (d)
             (let ((default-directory d))
               (and (file-exists-p ".git")
                    (if (file-directory-p ".git")
                        nil ; Regular repository
                      (with-temp-buffer
     ...
     (when root
       (let ((project-root
              (if (funcall check-submodule dir)
                  dir
                root)))
         (list 'vc 'Git project-root)))))

 (add-hook 'project-find-functions #'my-project-try-vc))
#+end_src
** Projectile
#+begin_src emacs-lisp :load no
(use-package
