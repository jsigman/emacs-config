...
Load machine-specific settings from ~local.el~ if it exists.
This file is not version controlled and should contain settings like:
- Machine-specific paths (e.g., Homebrew curl for gptel Bedrock)
- gptel backend configuration (AWS Bedrock or Anthropic API)
...
(use-package
 ...
 ;; Use quit-window: deletes window if possible, otherwise switches to another buffer
 ;; (delete-window can't work when vterm is the only window)
 ...
 ;; Exclude Claude Code buffers from vterm-toggle using the official mechanism
 ;; This prevents vterm-toggle from managing *claude-code[* buffers
 ;; vterm-toggle-togglable-buffer-functions was added in commit 6fff527 (Jan 2026)
 ;; and is the official way to filter buffers. This automatically applies to:
 ;; - Main toggle logic (M-T to show/hide)
 ;; - Buffer cycling (M-n/M-p)
 ;; - Window detection
 ;; - Mode hook buffer list management
 (add-to-list 'vterm-toggle-togglable-buffer-functions
              (lambda (buf)
                (not (string-prefix-p "*claude-code[" (buffer-name buf))))))
#+end_src
** Eat
#+begin_src emacs-lisp :load no
(use-package eat
  ...
  :config
  ;; Unbind keys to avoid conflicts with global bindings
  (define-key eat-mode-map (kbd "M-T") nil)
  (define-key eat-mode-map (kbd "M-o") nil)
  (define-key eat-semi-char-mode-map (kbd "M-T") nil)
  (define-key eat-semi-char-mode-map (kbd "M-o") nil)

  ;; Make ESC work with single press (map it to send C-c to the terminal)
  (define-key eat-semi-char-mode-map (kbd "<escape>")
    (lambda () (interactive) (eat-self-input 1 ?\C-c)))
...
;; Key bindings - Keep M-T for vterm-toggle, eat doesn't need global bindings
;; (eat is only used for Claude Code backend, not as a primary terminal)
...
(use-package
 ...
 ;; Only show dashboard when no files are specified on command line
 (setq initial-buffer-choice
       (lambda ()
         (if (> (length command-line-args) 1)
             ;; Files were specified, let Emacs open them normally
             t
           ;; No files specified, show dashboard
...
(defun my/eglot-capf ()
  (setq-local completion-at-point-functions
              (list #'tempel-expand  ; Tempel snippets (exact match)
                    (cape-capf-super
                     #'eglot-completion-at-point
                     #'cape-keyword  ; Provides language keywords like "pass", "def", etc.
                     #'cape-file))))

(add-hook 'eglot-managed-mode-hook #'my/eglot-capf)
#+end_src

*** Corfu/Tempel integration
#+begin_src emacs-lisp :load yes
;; Integrate Tempel snippets with Corfu completion UI
;; Tempel templates will appear as completion candidates in Corfu
;; NOTE: tempel-setup-capf is already configured in the Tempel section,
;; adding #'tempel-expand to completion-at-point-functions

;; Enable snippet support in Eglot so LSP servers can provide snippet completions
;; This advertises snippet capability to LSP servers and uses Tempel to expand them
(with-eval-after-load 'eglot
  (setq eglot-expand-snippet-function #'tempel-expand))

;; How it works:
;; 1. Tempel adds #'tempel-expand to completion-at-point-functions
;; 2. Corfu displays all CAPF candidates, including Tempel templates
;; 3. Eglot advertises snippet support to LSP servers
;; 4. When you select a completion, Tempel expands it with field navigation
#+end_src
...
;; Add tempel-collection for ready-made templates
(use-package tempel-collection
  :after tempel)
#+END_SRC
*** Custom Templates
#+begin_src emacs-lisp :load yes
;; Custom tempel templates
(defvar my-tempel-templates
  '(fundamental-mode ;; Available everywhere
    ...
    (log "import logging" n "logger = logging.getLogger(__name__)" n n)
    ...
    ;; File operations
    (rb "open(" (p "filepath") ", \"rb\")")
    (wb "open(" (p "filepath") ", \"wb\")")

    org-mode

    ;; Quick source blocks for specific languages
    (jp "#+begin_src jupyter-python" n r n "#+end_src" :post (org-edit-src-code))
    (py "#+begin_src ipython" n r n "#+end_src" :post (org-edit-src-code))
    (elisp "#+begin_src emacs-lisp" n r n "#+end_src" :post (org-edit-src-code))

    ;; Smart src block with language completion
    (src "#+begin_src "
         (p (completing-read "Language: "
                             (mapcar #'car org-babel-load-languages)
                             nil t)
            lang)
         ;; Add :results based on language type
         (cond
          ((member lang '("python" "bash" "shell" "sh")) " :results output")
          ((member lang '("dot" "plantuml" "ditaa")) " :file " (p "output.png"))
          (t ""))
         n r n "#+end_src"
         :post (org-edit-src-code)
         :ann "Smart src with language completion")

    ;; Remote bash block via TRAMP
    (bashremote "#+begin_src bash :dir /ssh:"
                (p "user" user) "@" (p "host" host) ":" (p "/path/to/dir/" dir)
                n r n "#+end_src"
                :post (org-edit-src-code)
                :ann "Bash on remote server")

    ;; Simple bash block
    (bash "#+begin_src bash :results output" n r n "#+end_src"
          :post (org-edit-src-code))

    ;; Bash block with :dir option
    (bashdir "#+begin_src bash :results output :dir " (p "./") n r n "#+end_src"
             :post (org-edit-src-code)
             :ann "Bash with custom directory")

    ;; Session-based src block (persistent REPL)
    (srcsess "#+begin_src "
             (p (completing-read "Language: " '("python" "R" "ruby" "julia")) lang)
             " :session " (p "*" session "*")
             " :results output"
             n r n "#+end_src"
             :post (org-edit-src-code)
             :ann "Src block with persistent session")

    ;; Tangled code block (exports to file)
    (srctangle "#+begin_src "
               (p (completing-read "Language: "
                                   (mapcar #'car org-babel-load-languages))
                  lang)
               " :tangle "
               (p (concat (file-name-base (or (buffer-file-name) "output"))
                          "." (pcase lang
                                ("python" "py")
                                ("emacs-lisp" "el")
                                ("bash" "sh")
               ...
               n r n "#+end_src"
               :post (org-edit-src-code)
               :ann "Tangled src block")

    ;; Named src block with automatic #+RESULTS
    (srcname "#+name: " (p "block-name" name) n
             "#+begin_src " (p "Language: " lang) n r n "#+end_src" n
             "#+RESULTS: " name n
             :post (org-edit-src-code))

    ;; Docker execution block
    (bashdocker "#+begin_src bash :results output"
                n "docker run --rm -it "
                (p "image" image) " bash -c '"
                n> r
                n> "'"
                n "#+end_src"
                :post (org-edit-src-code)
                :ann "Bash in docker container")

    ;; Property drawer (useful for org-roam, exports, etc.)
    (prop ":PROPERTIES:" n> ":" (p "KEY") ": " (p "value") n> ":END:")

    ;; Simple table starter (2x2)
    (table "| " p " | " p " |" n
           "|---+---|" n
           "| " p " | " p " |")

    ;; Meeting notes with current timestamp
    (meeting "* Meeting - " (p "Topic") n
             (format-time-string "[%Y-%m-%d %a %H:%M]") n n
             "** Attendees" n "- " p n n
             "** Notes" n r n n
             "** Action Items" n "- [ ] " p)

    ;; TODO with scheduled/deadline
    (todo "** TODO " (p "Task") n
          "SCHEDULED: <" (format-time-string "%Y-%m-%d %a") ">")
    (deadline "** TODO " (p "Task") n
              "DEADLINE: <" (format-time-string "%Y-%m-%d %a") ">")

    ;; Links with description
    (link "[[" (p "URL") "][" (p "description") "]]")
    (file "[[file:" (p "path") "][" (p "description") "]]")

    ;; Export blocks
    (html "#+begin_export html" n> r> n> "#+end_export")
    (latex "#+begin_export latex" n> r> n> "#+end_export"))
  "My custom tempel templates.")

;; Add custom templates to tempel sources after tempel is loaded
(with-eval-after-load 'tempel
  (add-to-list 'tempel-template-sources 'my-tempel-templates)

  ;; Add custom element (* count element...) for repeating elements
  ;; Used for dynamic tables and other repeated structures
  (defun tempel-repeat (elt fields)
    "Custom Tempel element for repeating.
Usage: (* COUNT ELEMENT...) where COUNT is evaluated and ELEMENT is repeated COUNT times."
    (pcase elt
      (`(* ,count . ,rest)
...
(use-package
 ...
 (add-to-list
  'eglot-server-programs
  '(dockerfile-mode . ("docker-language-server")))
 (add-to-list 'eglot-server-programs '(LaTeX-mode . ("texlab")))

 ;; Configure Ty LSP server settings for inlay hints
 (setq-default eglot-workspace-configuration
               '(:ty (:inlayHints (:callArgumentNames t
...
(use-package markdown-mode
  ...
  ;; Make markdown code blocks match org-mode styling
  (set-face-attribute 'markdown-code-face nil
                      :inherit 'org-block
                      :extend t)

  ;; Make the ``` fence lines look like org #+begin_src/#+end_src
  ;; Opening fence (``` emacs-lisp)
  ...
  ;; Ensure default mode for blocks without language specified
  ;; This enables syntax highlighting even when language is omitted
  (setq markdown-fontify-code-block-default-mode 'fundamental-mode))
#+END_SRC
...
(use-package markdown-preview-mode
  ...
  (setq markdown-preview-script-onupdate
        "document.querySelectorAll('code.mermaid').forEach(function(code) { var pre = code.parentElement; var div = document.createElement('pre'); div.className = 'mermaid'; div.textContent = code.textContent; pre.parentElement.replaceChild(div, pre); }); mermaid.run();")
...
#+end_src
** gptel
LLM client for Emacs. Supports AWS Bedrock (via SSO) and Anthropic API.

Backend configuration is done in ~local.el~ (machine-specific, not version controlled).
See the reference examples below for how to configure backends.

#+begin_src emacs-lisp :load yes
(use-package gptel
  :bind (("C-c g"   . gptel-send)           ;; Send region/buffer to LLM
         ("C-c G"   . gptel)                ;; Open chat buffer
         ("C-c M-g" . gptel-menu)           ;; Switch backend/model
         ("C-c r"   . gptel-rewrite-and-replace)) ;; Rewrite region
  :config
  ;; Use org-mode for chat buffers (nice formatting and structure)
  (setq gptel-default-mode 'org-mode))
#+end_src
** Claude Codes
*** vterm-anti-flicker-filter
Reduce flickering in vterm buffers, especially useful for Claude Code.
#+begin_src emacs-lisp :load yes
(use-package
 vterm-anti-flicker-filter
 :straight (:host github :repo "martinbaillie/vterm-anti-flicker-filter")
 :after vterm
 :config
 ;; The package auto-enables for all vterm buffers via vterm-mode-hook
 ;; You can toggle it manually with:
 ;; M-x vterm-anti-flicker-filter-toggle
 )
#+end_src
*** Claude Code IDE
#+begin_src emacs-lisp :load no
(use-package
 ...
 :after vterm-anti-flicker-filter
 :custom
 ...
 (global-set-key (kbd "C-c j") 'claude-code-ide-command-map)
 ...
 (:map
  ...
  ("t" . claude-code-ide-toggle-recent) ;; Toggle Claude window visibility
  ("@" . claude-code-ide-insert-at-mentioned) ;; Insert selected text
  ("d" . claude-code-ide-show-debug))) ;; Show debug buffer
#+end_src
*** Agent Shell
An alternative to claude-code-ide that uses shell-maker (comint-based) instead of vterm.
To switch between packages, set one to ~:load yes~ and the other to ~:load no~.

Both packages use the same ~C-c j~ prefix, so keybindings remain consistent.
#+begin_src emacs-lisp :load yes
(use-package agent-shell
  ...
  :custom
  ;; Use login-based auth (uses existing Claude CLI login)
  (agent-shell-anthropic-authentication
   (agent-shell-anthropic-make-authentication :login t))

  ;; Set Claude Code as the preferred agent
  (agent-shell-preferred-agent-config
   (agent-shell-anthropic-make-claude-code-config))

  ;; Dedicated side window on the right (like claude-code-ide)
  ...
  ;; Don't auto-enable completion (conflicts with Corfu)
  ;; Use M-x agent-shell-completion-mode to enable when needed
  (agent-shell-file-completion-enabled nil)

  :init
  ;; Create prefix keymap matching claude-code-ide pattern
  (define-prefix-command 'agent-shell-command-map)
  (global-set-key (kbd "C-c j") 'agent-shell-command-map)

  :config
  ;; Disable Corfu in agent-shell buffers to avoid conflicts
  (add-hook 'agent-shell-mode-hook
            (lambda () (corfu-mode -1)))

  :bind
  (:map agent-shell-command-map
   ("c" . agent-shell)                    ;; Start Claude
   ("n" . agent-shell-new-shell)          ;; New session
   ("t" . agent-shell-toggle)             ;; Toggle visibility
   ("s" . agent-shell-prompt-compose)     ;; Compose prompt
   ("i" . agent-shell-interrupt)          ;; Interrupt
   ("?" . agent-shell-help-menu)))        ;; Transient help menu
#+end_src
...
(use-package
 ...
 (setq treesit-auto-langs
       '(awk
         ...
         json
         make
         ;; markdown  ; Not supported by built-in Emacs treesit yet
         proto
         python
         ruby
         toml
